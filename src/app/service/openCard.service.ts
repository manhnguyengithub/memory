...
export class OpenCardService {
    ...
    openCard(currentPath: any, paramUrl: any) {
        ...
        else if (currentPath.includes('/memoryzone')) {
            if (paramUrl) {
                window.location.href = `${this.configService.urlMemoryzone}${paramUrl}`;
            } else {
                window.location.href = this.configService.urlMemoryzone;
            }
        }
        else {
            if (paramUrl) {
